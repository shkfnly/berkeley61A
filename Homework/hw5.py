...
def shuffle(cards):
    ...
    while count < len(cards):
        if count % 2 == 0 and i < n // 2:
            shuffled_cards.append(cards[i])
            i += 1
            count += 1
        elif count % 2 == 1 and start < n:
            shuffled_cards.append(cards[start])
            start += 1
            count += 1
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        nonlocal account_balance, key, attempts
        if attempts > 2:
            return 'Your account is locked. Attempts: ' + str(attempted)
        elif password == key and amount < account_balance:

...
type_tags = {Square: 'square', Rect: 'rectangle'}
...
apply_implementations =     {('area', 'square'): area_square,
                             ('perimeter', 'square'): perim_square, 
                             ('area', 'rectangle'): area_rect,
                             ('perimeter', 'rectangle'): perim_rect}
...
def apply(operator_name, shape):
    ...
    return apply_implementations[key](shape)
...
def union_set2(set1, set2):
    ...
    if empty(set1) and empty(set2):
        ...
        return Rlist(set1.first, union_set2(set1.rest, set2))
    elif set1.first > set2.first:
        return Rlist(set2.first, union_set2(set1, set2.rest))
    elif set1.first == set2.first:
        return Rlist(set1.first, union_set2(set1.rest, set2.rest))
...
def tree_to_ordered_sequence(s):
    ...
    """if s.left == None and s.right == None:
        return s
    elif s.left == Rlist.empty:
        return Rlist(s.entry, tree_to_ordered_sequence(s.right))
    elif s.right == Rlist.empty:
        return Rlist(s.entry, tree_to_ordered_sequence(s.left))
    else:
        return Rlist(tree_to_ordered_sequence(s.left), Rlist(s.entry, tree_to_ordered_sequence(s.right)))"""

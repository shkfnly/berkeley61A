...
(define (gcd m n)
  (cond ((equal? 0 (modulo m n)) (if (> m n) n m))
        ((> m n) (gcd n (modulo m n)))
        ((> n m) (gcd m (modulo n m)))
   )
   )
...
(define (test-q6)
  ...
  (assert-equal '(+ (* x y) (* y (+ x 3)))     (derive '(* (* x y) (+ x 3)) 'x)))
               

(define (derive-product exp var)
  (make-sum  (make-product (derive (multiplicand exp) var) (multiplier exp))

...
def extract_words(text):
    ...
    for letter in text:
        ...
        elif letter not in ascii_letters and letters != '':
            words.append(letters)
            letters = ''
    if letters != '':
        words.append(letters)
    return words # You may wish to change this line.
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    value, sentiments, words = 0, 0, tweet_words(tweet)
    for word in words:
    ...
    if sentiments == 0:
        ...
        return make_sentiment(value/sentiments)
...
def find_centroid(polygon):
    ...
    """def calculate(kind, polygon):
        if kind == 'lat':
            for _ in range(len(polygon - 1)):
                value += (latitude(polygon[_]) + latitude(polygon[_ + 1])) * (latitude(polygon[_]) * longitude(polygon[_ + 1])) - (latitude(polygon[_ + 1]) * longitude(polygon[_]))
            return (1/(6 * area_poly)) * lat_value
        elif kind =='lon':
            for _ in range(len(polygon - 1)):
                lon_value += (longitude(polygon[_]) + longitude(polygon[_ + 1])) * (latitude(polygon[_]) * longitude(polygon[_ + 1])) - (latitude(polygon[_ + 1]) * longitude(polygon[_]))
        return (1/(6 * area_poly)) * lat_value"""
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        for state in us_states:
            center = find_state_center(us_states[str(state)])
            if geo_distance(location, center) < distance:
                closest = state
                distance = geo_distance(location, center)
        if closest in tweets_by_state.keys():
            tweets_by_state[closest].append(tweet)
        else:
            tweets_by_state[closest] = []      
            tweets_by_state[closest].append(tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        value, num, avg = 0, 0, 0
        for i in range(len(tweets_by_state[state])):
            senti = analyze_tweet_sentiment(tweets_by_state[state][i])()
            if senti is not None:
                value += senti
                num += 1
        if num != 0 and value != 0:
            avg = value/num
            averaged_state_sentiments[state] = avg
